...
class BenchHandler:
    ...
    def _on_bench_state_callback(self, bench_state):
        ...
        if len(self.state_buffer) >= 30:
            ...
            # Create a sinus that is half the amplitude of total range of motion
            sinus_amplitude = abs(TOP_ANGLE_VALUE - BOT_ANGLE_VALUE) / 5

            # Create a sinus with the period of 10 seconds. Each tick is 0.01 seconds
            sinus_period = 1000
            sin_val = sinus_amplitude * torch.sin(2 * np.pi * torch.tensor(list(range(current_tick, current_tick + 30))) / sinus_period)
            sin_val = sin_val.unsqueeze(0)

            # add sinus to desired future outputs
            desired_future_outputs = desired_future_outputs + sin_val
